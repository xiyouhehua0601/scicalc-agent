...
真实结果取决于用的模型，跑 `python eval.py` 就能得到你自己的数字，会自动存到 `results/eval.json`。用 DeepSeek-chat 大概的样子（仅供参考）：

| 题型 | 结果 |
|------|------|
| 单位换算 / 算术 / 几何 | 基本全对，一步 calculator 搞定 |
| 运动学 / 功与能 | 多数对，偶尔公式记错但 python 能兜底 |
| 材料力学（需要公式推导） | 最容易错，暴露 LLM 公式记忆不可靠 |

失败的题里，`parse_error` 和 `wrong_answer` 是主要来源——说明**瓶颈在"模型会不会用工具、公式记没记对"，而不是工具本身**。这就是这个项目想量化出来的东西。
